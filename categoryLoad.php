...
if ($cat != "0" && $bd=="0") {
    $query .= " WHERE   product.category_cat_id='" . $cat . "' AND product.status_status_id='1' ";
    if (!empty($keyword)) {
        $query .= " AND product.title LIKE '%" . $keyword . "%'";
    }
     $query .="ORDER BY `datetime_added` DESC LIMIT 8 OFFSET $offset";

} elseif ($bd=="0") {
    $query .= " WHERE product.status_status_id = '1' ";
    if (!empty($keyword)) {
        $query .= " AND product.title LIKE '%" . $keyword . "%'";
    }
    $query .="ORDER BY `datetime_added` DESC LIMIT 8 OFFSET $offset";
} else {
    $query .= " INNER JOIN model_has_brand ON product.model_has_brand_id = model_has_brand.id WHERE model_has_brand.brand_brand_id='".$bd."' AND product.status_status_id='1' ";

    if (!empty($keyword)) {
        $query .= " AND product.title LIKE '%" . $keyword . "%'";
    }
    $query .="ORDER BY product.datetime_added DESC LIMIT 8 OFFSET $offset";
}
            ...
            <div class="card">
                <div class="card-im  Wproduct" style=" background-image: url('<?php echo ($d['image_1']) ?>');" onmouseover="this.style.backgroundImage = 'url(\'<?php echo ($d['image_2']) ?>\')';" onmouseout="this.style.backgroundImage = 'url(\'<?php echo ($d['image_1']) ?>\')';">
                    ...
                    <div class="card-body card-effect d-flex flex-column">
                        ...
                        <div class="card-button card-button-container d-flex flex-row justify-content-center overflow-hidden">
